...
public class Experiment {
	...
	/**
	 * Executes the experiment from the provided ExperimentInfo
	 * object. 
	 */
	public void execute(){
		// TODO Auto-generated constructor stub
...
//	''' Initialises the experiment. It is also responsible for reading the 
...
//	        #------------------------ Initialise raw data handler for the experiment
...
//	            sampledPatients, nonsampledPatients = self.__dataHandler.getBootstrapSampleOfPatients(sampleRatio, duplicates)
//	            assignedCore = i%noOfCores
//	            assignedLock = locksForCores[assignedCore]
//	            assignedLock.acquire()
//	            assignedLock.release()
//	            #experimentRun(assignedLock, self.__expInfo, i, objOutput, sampledPatients, nonsampledPatients)
//	            expRunProcess = Process(target=experimentRun, args=(assignedLock, self.__expInfo, i, objOutput, sampledPatients, nonsampledPatients))
//	            expRunProcessList[i] = expRunProcess
//	            expRunProcess.start()
//	            if (i%10==500):
//	                print i, 'active =', len(multiprocessing.active_children()), 'duration=%f'%(timeit.default_timer() - start_time)
...
//	        
//	        #--------------------------- Write duration info to the main output file
//	        relResultDir = self.__expInfo.getParamVal(PARAM_NAME)
//	        expName      = self.__expInfo.getParamVal(PARAM_NAME)
//	        outputFilename = '%s_main.txt'%(expName)
//	        outputFilepath = PATH_FOLDER_RESULT + os.path.sep + relResultDir + os.path.sep + outputFilename
//	        
//	        #--------------------------------------------------------- Write to file
