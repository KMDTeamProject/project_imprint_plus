...
public class MultipleExperiments {
	 ...
	 */
	public static Collection<Experiment> readExperiments(String _expRelDir,
			Collection<String> _expFilenames) {
	...
	//
	//
	// ''' Generates a list of Experiment objects. The experiments are run
	// for
	...
	// #------------------- Save old path and change path to settings
	// directory
	// oldPath = os.getcwd()
	// os.chdir(expDir)
	...
	//
	// ''' Reads information for multiple experiments and run them one by
	// one '''
	// def runMultipleExperiments(self, _listOfExpInfo):
	// print 'Running Multiple Experiments: %d experiments object in
	// total.'%len(_listOfExpInfo)
	// success = 0
	...
	// #print e
	// #raise e
	// print '%d/%d experiments successfully
	// completed.'%(success,len(_listOfExpInfo))
	//
	// ''' Reads information for multiple experiments and run them one by
	// one '''
