	...
	public String predict(LongitudinalObject _obj)
		...
		for(int i = 0; i <= l_series.size()-n; i++) {
            if(nGram.containsKey(l_series.get(i) + " " + l_series.get(i+1))) {
                ...
                if(nGram.get(l_series.get(i)+" "+l_series.get(i+1)).containsKey(l_series.get(i+2))) 
                {
                    double v = nGram.get(l_series.get(i) + " " + l_series.get(i+1)).get(l_series.get(i+2));
                    v++;
                    nGram.get(l_series.get(i) + " " + l_series.get(i+1)).put(l_series.get(i+2), v);
                } else {
                    nGram.get(l_series.get(i) + " " + l_series.get(i+1)).put( l_series.get(i+2), 1.0);
                }
            } else {

                nGram.put(l_series.get(i)+" "+l_series.get(i+1), create(l_series.get(i+2)));
            }
        }
		...
		last_label =  l_series.get(last_label_index)+" "+l_series.get(last_label_index+1);
		
		/*
		 * Search the nGram for the label with the best confidence while current label is 
		 * same as last n-label 
		 */
